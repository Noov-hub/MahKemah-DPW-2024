    ...
    <div class="col-lg-12" style="margin:5px">
        <div class="member d-flex align-items-center">
            <span class="border" style="margin:10px">
                <div class="pic">
                    <img src="{{ asset('img/sepatu/' . $shoe->image) }}" class="card-img-top" alt="...">
                </div>
            </span>
            <div class="member-info">
                <div class="card-body">
                    <h1 class="card-title">{{ $shoe->brand }}</h1>
                    <p>Harga Sewa per Barang: Rp.{{ number_format($shoe->price, 0, ',', '.') }}</p>
                    {{-- <p>Jumlah: {{ $jumlah }}</p> --}}
                    <p>Total Harga: Rp.{{ number_format($totalHarga, 0, ',', '.') }}</p>
                    <form action="{{ route('konfirmasi_shoe') }}" method="POST">
                        @csrf
                        <input type="hidden" name="shoe_id" value="{{ $shoe->id }}">
                        <input type="hidden" name="harga" value="{{ $shoe['price'] }}">
                    ...
                    <a href="{{ route('shoe_details', ['user_id' => auth()->user()->id, 'id' => $shoe['id']]) }}" style="margin:5px">Kembali</a>
                </div>
            </div>
        </div>
    </div>
